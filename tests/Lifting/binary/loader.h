...
namespace BinaryLoader {
    ...
    public:
        ...
        struct FuncEntry {
            ...
            bool elf_entry;
            FuncEntry(uintptr_t __entry, std::string __func_name, bool __elf_entry) : entry(__entry), func_name(__func_name), elf_entry(__elf_entry) {}
            bool operator<(const FuncEntry &rhs) { return entry < rhs.entry; }
        };

        void LoadELF();
        std::tuple<uint8_t*, uint64_t, uintptr_t> GetTextSection();
